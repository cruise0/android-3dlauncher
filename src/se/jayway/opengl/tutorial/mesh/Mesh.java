...
public class Mesh {
	...
	public boolean bCulFace = true;
	
	private PressedHandler mpHandler = null;

	public void setPressedHandler( PressedHandler pHandler )
	{
		mpHandler = pHandler;
	}
	/**
	 ...
	 * @param bCheck 
	 */
	public void draw(GL10 gl, Projector proj, boolean bCheck, float scrX, float scrY) {
		...
		if( bCheck )
		{
			...
			for( int i = 0 ; i < mVerticesBuffer.capacity() / 3 ; i++ )
			{
				float[] mScratch = new float[8];
				mScratch[0] = mVerticesBuffer.get( 3 * i );
				mScratch[1] = mVerticesBuffer.get( 3 * i + 1 );
				mScratch[2] = mVerticesBuffer.get( 3 * i + 2 );
				mScratch[3] = 1.0f;
				
				proj.project( mScratch , 0 , mScratch , 4);
				
				mScratch[5] = proj.getViewHeight() - mScratch[5];
				
				float nowX = mScratch[4];
				float nowY = mScratch[5];
				xMin = Math.min( xMin , nowX );
				xMax = Math.max( xMax , nowX );
				yMin = Math.min( yMin , nowY );
				yMax = Math.max( yMax , nowY );
			}
			
			if( inBox( xMin , xMax , yMin , yMax , scrX , scrY ) )
			{
				
				if( mpHandler != null )
					mpHandler.Pressed( scrX , scrY );
			}
	...
	private boolean inBox(float xMin, float xMax, float yMin, float yMax,
			float scrX, float scrY) {
		// TODO Auto-generated method stub
		
		if( scrX < xMin || scrX > xMax || scrY < yMin || scrY > yMax )
			return false;
		return true;
	}
